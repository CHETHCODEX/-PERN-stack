...
async function getQuotations(req, res, next) {
    try {
        const quotations = await db_1.prisma.quotation.findMany({
            orderBy: { id: 'desc' },
            include: {
                customer: true,
                enquiry: {
...
async function createQuotation(req, res, next) {
    try {
        ...
        const calcResult = (0, calculations_1.calculateQuotationTotals)(items);
        const quotationNumber = (0, numberGenerator_1.generateSequenceNumber)('QUO');
        // Transaction: Create quotation + items, update enquiry status to QUOTED
        const quotation = await db_1.prisma.$transaction(async (tx) => {
...
async function convertQuotationToSalesOrder(req, res, next) {
    try {
        ...
        const count = await db_1.prisma.salesOrder.count();
        const orderNumber = `SO-${new Date().getFullYear()}-${String(count + 1).padStart(4, '0')}`;
        const salesOrder = await db_1.prisma.salesOrder.create({
            data: {
                orderNumber,
