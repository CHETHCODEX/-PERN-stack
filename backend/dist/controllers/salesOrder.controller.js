...
async function getSalesOrderById(req, res, next) {
    try {
        ...
        const order = await db_1.prisma.salesOrder.findUnique({
            where: { id },
            include: {
                customer: true,
                quotation: {
        ...
        `;
            ...
            for (const item of order.items) {
                ...
                if (available < item.quantity) {
                    const err = new Error(`Insufficient stock for Product ID ${item.productId}. Available: ${available}, Requested: ${item.quantity}.`);
                    err.statusCode = 400;
...
async function dispatchSalesOrder(req, res, next) {
    try {
        ...
        const sortedProductIds = Array.from(new Set(order.items.map((i) => i.productId))).sort((a, b) => a - b);
        const dispatchNumber = (0, numberGenerator_1.generateSequenceNumber)('DSP');
        const dispatchRecord = await db_1.prisma.$transaction(async (tx) => {
            // Lock rows
            const lockedInventories = await tx.$queryRaw `
          SELECT product_id, physical_qty, reserved_qty 
          FROM pern_erp.inventory 
