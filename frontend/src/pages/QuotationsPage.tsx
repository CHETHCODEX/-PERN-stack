      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Generate pricing with item discounts and GST breakdown. Move through DRAFT â†’ SENT â†’ ACCEPTED.
          </p>
        </div>
        <button
          onClick={() => {
                ...
                {quotations.map((quo) => (
                  <tr key={quo.id} className="hover:bg-slate-50/80 transition">
                    ...
                    <td className="px-6 py-4 text-right font-semibold text-slate-900">
                      â‚¹{Number(quo.grandTotal).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                    </td>
                      ...
                      <div className="flex items-center justify-end space-x-2">
                        ...
                        {quo.status === 'ACCEPTED' && (
                          quo.salesOrder ? (
                            ...
                            >
                              <span>Convert to Sales Order â†’</span>
                            </button>
                          )
                        )}
                  ...
                  <table className="min-w-full divide-y divide-slate-200 text-xs">
                    <thead className="bg-slate-50 font-semibold text-slate-600">
                      <tr>
                        ...
                        <th className="px-3 py-2 text-right">Unit Price (â‚¹)</th>
                        ...
                        return (
                          <tr key={idx}>
                            ...
                            <td className="px-3 py-2 text-right font-mono font-semibold text-slate-900">
                              â‚¹{line.toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                            </td>
                          </tr>
                        );
                      })}
                ...
                <div className="mt-3 flex justify-end">
                  <div className="bg-slate-50 border border-slate-200 rounded-lg px-4 py-2 text-right">
                    ...
                    <span className="text-base font-bold text-slate-900 font-mono">
                      â‚¹{Number(calculatePreviewTotal()).toLocaleString('en-IN', { minimumFractionDigits: 2 })}
                    </span>
                  </div>
                </div>
              </div>
